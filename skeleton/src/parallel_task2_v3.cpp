// Experimental optimization on top of the current, verified
// parallel_task2.cpp design (kept separate; that file is untouched).
//
// Idea: in process_positions_parallel, every occurrence pays a
// remove_edge(state, position, frequency) call to remove the CURRENT
// round's own pair (left_token,right_token) from the shared registry. But
// by construction (R5.4: overlapping occurrences merge left-to-right,
// exhaustively), every word that contributes at least one successful merge
// this round has ALL of its live edges of (left_token,right_token)
// consumed this round -- there is no way for a word to end this round with
// some live edges of the pair remaining. That means, instead of N separate
// remove_edge calls (one per occurrence) each re-deriving "does this
// word's group_count hit zero", we can:
//   - accumulate the total frequency removed (already done for
//     token_count, via reduction -- reused here) and the count of DISTINCT
//     WORDS that merged at least once this round (also a reduction),
//   - apply ONE aggregate update to the pair_state for
//     (left_token,right_token): count -= total_frequency,
//     word_count -= words_merged (which will drive word_count to exactly
//     zero, since every word with any live edge of this pair necessarily
//     merges it away this round),
//   - and recycle each word's group (captured once, from the first
//     occurrence processed for that word, before any mutation) onto the
//     free list, instead of doing it via remove_edge's own bookkeeping.
//
// This removes roughly 1/3 of the sequential replay's remove_edge calls
// (the unconditional "own pair" one) without touching the trickier
// left/right-neighbour removals or the add side at all, and without any
// locking -- same "aggregate what's provably safe to aggregate" strategy
// that already worked for token_count. Whether the saving is big enough to
// matter in practice (it should help most when a round has many occurrences
// clustered in relatively few words, less when most words contribute
// exactly one occurrence each) needs real data -- untested on Spartan as
// of writing.
...
struct MergeEvent {
    ...
    // Set only for self-pair rounds (left_token == right_token), where the
    // "own pair" batching is bypassed -- see process_positions_parallel.
    // Tells the replay loop to remove_edge(position) itself, same as the
    // sequential reference / non-batched path.
    bool needs_own_removal = false;
};

void process_positions_sequential(task2_state& state,
                                  const std::vector<u32>& positions,
                                  u32 left_token, u32 right_token,
                                  u32 merged_token) {
...
// Same word-bucketed parallel structural pass as parallel_task2.cpp, plus
// the "own pair" batching described at the top of this file. best_state is
// the pair_state index for (left_token,right_token) -- the pair chosen for
// this round -- needed to apply the batched update directly.
void process_positions_parallel(task2_state& state,
                                const std::vector<u32>& positions,
                                u32 left_token, u32 right_token,
                                u32 merged_token, u32 best_state) {
    ...
    // The "own pair" batching below assumes a removal via right_position
    // can never target the SAME group as the round's own pair. That's true
    // whenever left_token != right_token: right_position's forward pair is
    // (right_token, after_token), which only equals (left_token,right_token)
    // if right_token == left_token too. For a self-pair round (e.g.
    // merging (a,a) inside "aaaa"), that collision is real -- an
    // overlapping occurrence's right-neighbour removal can land on the
    // exact group the batching already claimed. Bypass the optimization
    // for that case and fall back to the same per-occurrence remove_edge
    // the original design uses; every other round still benefits.
    const bool self_pair = (left_token == right_token);

    const int num_threads = omp_get_max_threads();
    std::vector<std::vector<MergeEvent>> local_events(num_threads);
    std::vector<std::vector<u32>> local_own_groups(num_threads);
    ...
    #pragma omp parallel
    {
        ...
        std::vector<u32>& own_groups = local_own_groups[tid];
        events.reserve(positions.size() / static_cast<std::size_t>(num_threads) + 16);

        #pragma omp for schedule(dynamic, 64) reduction(+:total_frequency, words_merged)
        for (std::size_t idx = 0; idx < active_words.size(); ++idx) {
            ...
            bool word_had_merge = false;
            u32 own_group_for_word = no_position;
            ...
            for (u32 position : by_word.at(word)) {
                ...
                if (!self_pair && !word_had_merge) {
                    // Every live occurrence of (left_token,right_token) in
                    // this word shares the same group (one group per
                    // (pair,word), by construction) -- capture it once,
                    // from the first occurrence, before any mutation.
                    own_group_for_word = state.edge_group[position];
                    if (own_group_for_word == no_position) {
                        std::abort();
                    }
                    word_had_merge = true;
                }
                ...
                ev.needs_own_removal = self_pair;
                ev.left_position =
            ...
            if (word_had_merge) {
                ++words_merged;
                own_groups.push_back(own_group_for_word);
            }
        }
    }

    // Batched update for this round's own pair: every word that merged at
    // least once necessarily drains ALL of its live edges of this pair
    // (R5.4), so word_count always ends up decreasing by exactly
    // words_merged, and count by exactly total_frequency -- no need to
    // discover this per occurrence via remove_edge. Skipped entirely for
    // self-pair rounds: those never set word_had_merge (see above), so
    // words_merged stays 0 and total_frequency here would double-count
    // against the per-event remove_edge(position) calls the replay loop
    // does instead (ev.needs_own_removal).
    if (!self_pair) {
        pair_state& own_pair = state.pair_states[best_state];
        if (own_pair.count < total_frequency) {
            std::abort();
        }
        own_pair.count -= total_frequency;
        if (own_pair.word_count < words_merged) {
            std::abort();
        }
        own_pair.word_count -= static_cast<u32>(words_merged);
        own_pair.last_word = no_position;
        own_pair.last_group = no_position;
    }
    for (const std::vector<u32>& own_groups : local_own_groups) {
        for (u32 group : own_groups) {
            state.group_count[group] = state.free_head;
            state.free_head = group;
        }
    }
    ...
    for (const std::vector<MergeEvent>& events : local_events) {
        for (const MergeEvent& ev : events) {
            ...
            // For self-pair rounds, the batching above is bypassed, so
            // this event's own removal wasn't accounted for anywhere else
            // -- replay it here, same as the non-batched design.
            if (ev.needs_own_removal) {
            ...
            if (ev.left_position != no_position) {
                ...
                add_edge(state, ev.left_position, state_id, ev.word, ev.frequency);
            }
            if (ev.after_position != no_position) {
                const u32 state_id =
    ...
    for (;;) {
        ...
        if (positions.size() >= kParallelThreshold &&
            ...
            process_positions_parallel(state, positions, left_token,
                                       right_token, merged_token, best_state);
        } else {
