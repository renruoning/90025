// Experimental redesign of Task2's bookkeeping data structure.
//
// The original task2.cpp / parallel_task2.cpp design tracks, for each
// (pair, word), a "group" -- an index into shared arrays, cached via a
// single last_word/last_group slot per pair. That single-slot cache is
// fundamentally sequential: it can only remember one word at a time, so it
// breaks the moment two different words concurrently want a group for the
// same pair. Every attempt to parallelize around it either broke
// correctness (see benchmark_notes.txt, attempt #2) or would have required
// locking, at which point there was little reason to keep the group/free-
// list machinery at all.
//
// This version drops groups entirely. Each pair is a direct entry in a
// hash map (bpe::(pair key) -> PairRecord), and each PairRecord carries its
// own small word -> live-edge-count map. Removing/adding a pair's edge for
// a word is then a direct map operation, no group indices, no free list.
//
// The remove+add for one occurrence still has to happen together, in the
// original relative order, to stay correct for chained/overlapping
// occurrences within one word (see the note above and the R5.4 rule) --
// that requirement isn't a group-system artifact, it's inherent to the
// algorithm. So this version keeps remove+add interleaved, protecting the
// shared pair registry with a single critical section around each of the
// two bookkeeping calls, while the structural linked-list surgery (which
// is the bulk of the per-occurrence work, and is word-exclusive) stays
// completely lock-free and runs in parallel across words.
//
// Whether this is actually faster than the current parallel_task2.cpp is
// an open question -- untested on Spartan as of writing. Locking is not
// free, and every occurrence still needs the registry lock twice. The
// hope is that the *per-call* cost while holding the lock is cheaper here
// (a couple of hash lookups, no free-list bookkeeping) than what the
// current design's fully-deferred sequential replay pays per event, enough
// to offset the lock overhead. Needs real numbers, on real data, to know
// either way.
...
// One entry per distinct (left,right) pair ever seen. word_edges tracks,
// per word, how many live adjacent occurrences of this pair currently
// exist in that word -- this replaces the group/group_count system
// one-for-one, just as a direct hash map instead of an indexed free list.
struct PairRecord {
    u64 count = 0;
    u32 word_count = 0;
    u64 fingerprint = 0;
...
// All mutating access to state.pairs (structural insertion included) must
// happen while holding the pair_registry critical section once we're past
// build_state_v2 (i.e. once the parallel merge loop starts). build_state_v2
// itself is single-threaded, so it's exempt.
PairRecord& get_pair(State2& s, u64 key) {
    auto [it, inserted] = s.pairs.try_emplace(key);
    if (inserted) {
        it->second.fingerprint = text_fingerprint(s.vocabulary[pair_left(key)],
                                                   s.vocabulary[pair_right(key)]);
...
void add_edge_v2(State2& s, u32 left, u32 right, u32 word, u32 position,
                 u64 frequency) {
    PairRecord& rec = get_pair(s, pack_pair(left, right));
...
struct QueueHeap {
    ...
    const QueueEntry& top() const { return data.front(); }
...
bool QueueCompare::operator()(const QueueEntry& left,
    ...
    const std::string left_text = state->vocabulary[pair_left(left.key)] +
                                  state->vocabulary[pair_right(left.key)];
    const std::string right_text = state->vocabulary[pair_left(right.key)] +
                                   state->vocabulary[pair_right(right.key)];
...
constexpr u64 kNoKey = ~0ULL;  // pack_pair(no_position, no_position)
...
void build_state_v2(const std::vector<CharSplit>& splits, State2& state) {
    ...
    for (u32 word = 0; word < splits.size(); ++word) {
        ...
        for (u32 position = first; is_live(state, position);
            ...
            add_edge_v2(state, state.token[position], state.token[next_position],
                       word, position, split.count);
        }
    }
    state.born_keys.clear();  // build_state's own insertions don't need queuing here
}

void process_positions_v2(State2& state, const std::vector<u32>& positions,
                          u32 left_token, u32 right_token, u32 merged_token,
                          bool use_parallel) {
    ...
    auto process_word = [&](u32 word) {
        ...
        for (u32 position : by_word.at(word)) {
            ...
            // Structural linked-list surgery: word-exclusive, lock-free.
            ...
            {
                if (has_left) {
                    add_edge_v2(state, left_neighbor, merged_token, word,
                               left_position, frequency);
                }
                if (has_after) {
                    add_edge_v2(state, merged_token, after_neighbor, word,
                               position, frequency);
                }
            }
        }
...
void run_merge_loop_v2(State2& state) {
    ...
    for (;;) {
        ...
        const bool use_parallel =
            positions.size() >= kParallelThreshold && omp_get_max_threads() > 1;
        process_positions_v2(state, positions, left_token, right_token,
                            merged_token, use_parallel);
...
// Not wired into the real parallel_task2 entry point -- deliberately kept
// separate so the existing, verified parallel_task2.cpp is untouched.
// Call this directly from a test/benchmark driver to exercise it.
